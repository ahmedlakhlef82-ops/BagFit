...
export async function createAirlineAction(data: any) {
  await verifyAdmin()
  const supabase = await createClient()

  const { data: existing } = await supabase.from('airlines').select('id').or(`iata_code.eq.${data.iata_code},icao_code.eq.${data.icao_code}`).maybeSingle()
...
export async function updateAirlineAction(id: string, data: any) {
  await verifyAdmin()
  const supabase = await createClient()

  if (data.iata_code || data.icao_code) {
     const { data: existing } = await supabase.from('airlines').select('id').neq('id', id).or(`iata_code.eq.${data.iata_code},icao_code.eq.${data.icao_code}`).maybeSingle()
     if (existing) return { error: 'Airline with this IATA or ICAO code already exists.' }
  }
...
export async function importAirlinesCSV(airlinesData: any[]) {
   ...
   if (validAirlines.length === 0) return { error: 'No valid airlines found in CSV' };

   const { error } = await supabase.from('airlines').upsert(validAirlines, { onConflict: 'iata_code' });
